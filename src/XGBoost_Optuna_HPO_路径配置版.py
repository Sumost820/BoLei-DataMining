...
DATA_FILE = PROJECT_ROOT / "data" / "JiangYi" / "任务特征和标签-JiangYi_层次聚类.json"

# 输出：该模型的超参数寻优结果
OUTPUT_DIR = PROJECT_ROOT / "target" / "HPO_XGBoost"
...
def suggest_params(trial: optuna.Trial, seed: int, n_jobs: int) -> Dict:
    return {
        ...
        "eval_metric": "rmse",
        "tree_method": "hist",
        "n_estimators": trial.suggest_int("n_estimators", 250, 1800, step=50),
        ...
        "reg_lambda": trial.suggest_float("reg_lambda", 0.10, 100.0, log=True),
        "random_state": seed,
        "n_jobs": n_jobs,
        "verbosity": 0,
    }
